        ...
        <div className="min-h-screen bg-card max-w-lg mx-auto pt-8 pb-24 px-4 relative">
            ...
            <div className="flex flex-col items-center gap-2 mb-8">
                {settings.logoDataUrl ? (
                    <img src={settings.logoDataUrl} alt={settings.studioName} className="w-12 h-12 rounded-xl object-contain shadow-md" />
                ) : (
                    <div className="w-12 h-12 bg-indigo-500 rounded-xl flex items-center justify-center shadow-md">
                        <User className="w-6 h-6 text-white" />
                    </div>
                )}
                <h1 className="text-lg font-black text-primary tracking-tight">{settings.studioName}</h1>
                <p className="text-xs font-bold text-muted opacity-50 uppercase tracking-[0.15em]">{t.addStudentShort}</p>
            </div>
