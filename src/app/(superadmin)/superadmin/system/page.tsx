        ...
        <div className="space-y-6 animate-fade-up pb-20">
            ...
            {loading ? (
                <div className="py-24 flex flex-col items-center justify-center space-y-4 bg-white/50 rounded-2xl border border-black/5">
                    ...
                    <p className="text-[10px] font-black text-zinc-400 uppercase tracking-widest">{lang === 'ka' ? 'სისტემური სტატისტიკის განახლება...' : 'Refreshing system stats...'}</p>
                </div>
            ) : (
                <>
                    ...
                    <div className="bg-white/95 border border-black/10 dark:border-border-subtle rounded-2xl p-5 shadow-sm">
                        <h2 className="text-sm font-black text-primary mb-3 uppercase tracking-tight">{lang === 'ka' ? 'ღრუბლოვანი რეესტრი' : 'Cloud Registry'} ({slugs.length})</h2>
                        <div className="flex flex-wrap gap-2">
                            ...
                            {slugs.length === 0 && <p className="text-xs text-zinc-400 font-black uppercase opacity-20">{lang === 'ka' ? 'სისტემა ცარიელია' : 'System is empty'}</p>}
                        </div>
                    </div>

                    <div className="space-y-3">
                        <h2 className="text-sm font-black text-muted uppercase tracking-widest opacity-40">{lang === 'ka' ? 'ადმინისტრაციული ქმედებები' : 'Administrative Actions'}</h2>
                        {tools.map(tool => (
                            <div key={tool.id} className={cn('border rounded-2xl p-5 flex items-center gap-4 shadow-sm', tool.bg)}>
                                ...
                                {done === tool.id ? (
                                    ...
                                    <div className="flex items-center gap-2">
                                        <button onClick={() => doAction(tool.id, tool.action)} className="px-3 py-2 bg-rose-600 hover:bg-rose-700 text-white text-xs font-black rounded-xl transition-all shadow-lg shadow-rose-600/20">{lang === 'ka' ? 'დიახ, წაშალე' : 'Yes, Delete'}</button>
                                        <button onClick={() => setConfirmingId(null)} className="px-3 py-2 bg-white text-muted text-xs font-black rounded-xl border border-black/5 hover:bg-black/5 transition-all shadow-sm">{lang === 'ka' ? 'გაუქმება' : 'Cancel'}</button>
                                    </div>
                                ) : (
