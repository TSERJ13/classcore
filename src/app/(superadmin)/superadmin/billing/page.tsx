...
export default function BillingPage() {
    ...
    return (
        <div className="space-y-6 animate-fade-up">
            <div>
                <h1 className="text-2xl font-black text-primary tracking-tight">{lang === 'ka' ? 'გლობალური ფინანსები' : 'Global Billing'}</h1>
                <p className="text-sm text-muted mt-1">{lang === 'ka' ? 'შემოსავლების მიმოხილვა ყველა სტუდიაში' : 'Revenue overview across all studios'}</p>
            </div>

            {loading ? (
                <div className="py-24 flex flex-col items-center justify-center space-y-4">
                    <div className="w-12 h-12 border-4 border-indigo-500/10 border-t-indigo-500 rounded-full animate-spin" />
                    <p className="text-[10px] font-black text-zinc-400 uppercase tracking-widest">{lang === 'ka' ? 'ფინანსური მონაცემების სინქრონიზაცია...' : 'Syncing financial records...'}</p>
                </div>
            ) : (
                <>
