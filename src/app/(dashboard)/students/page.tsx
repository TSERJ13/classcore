...
import { Search, UserPlus, Users, Calendar, Phone, ShieldAlert, Heart, AlertTriangle, Link2, Check, Edit2, Trash2, Zap, AlertCircle, Filter, MessageCircle, Loader2 } from 'lucide-react';
import { useT } from '@/contexts/LanguageContext';
import { useUser } from '@/hooks/useUser';
import { useStudio } from '@/contexts/StudioContext';
import { cn, getInitials, isExpiringSoon, calculateAge, formatDate, formatPhoneDisplay } from '@/lib/utils';
...
function StudentsPageInner() {
    ...
    const [smsRecipient, setSmsRecipient] = useState<StudentRow | null>(null);
    ...
    const visibleGroupIds = useMemo(() => {
        if (!isTeacherRole(profile?.role)) return undefined;
        return getVisibleGroupIds(profile as any, (settings.staff || []) as any, groups as any) || undefined;
    }, [profile, settings.staff, groups]);
    ...
    async function handleSave(formData: Partial<Student>) {
        if (!editing) {
            const dup = await checkDuplicate.mutateAsync({
                full_name: formData.full_name || '', phone: formData.phone || '', birth_date: formData.birth_date,
            });
            if (dup) {
        ...
        await saveStudent.mutateAsync({ ...formData, id: formData.id || editing?.id } as any);
        setModalOpen(false);
    }
                        ...
                        <div key={student.id}
                            ...
                            <div className="flex items-start gap-4">
                                ...
                                <div className="w-14 h-14 rounded-2xl flex-shrink-0 overflow-hidden border-2 border-surface group-hover:border-[#6d28d9]/20 transition-all relative">
                                    ...
                                    {!!student.photo_url && (
                                        <img
                                            src={student.photo_url as string}
                                    ...
                                    <div className="flex flex-wrap items-center gap-2 mb-1">
                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div className="flex items-center gap-1.5 ml-auto sm:ml-0">
                                                {!!student.social_links && Object.entries(student.social_links as Record<string, string>).map(([platform, link]) => {
                                                    if (!link) return null;
                                                    ...
                                                    const p = icons[platform as keyof typeof icons] || { color: 'bg-surface text-muted', icon: <Link2 className="w-3 h-3" /> };
                                                    return (
                                                        <a key={platform} href={link} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()}
                                                            className={cn("w-7 h-7 rounded-lg flex items-center justify-center border transition-all hover:scale-110 active:scale-95 shadow-sm", p.color)}>
                                                            {p.icon}
                                                        </a>
                                                    );
                                                })}
                                            ...
                                            <div className="flex flex-wrap items-center gap-x-3 gap-y-2 pt-2">
                                                ...
                                                {!!student.birth_date && (
                                                    <div className="flex items-center gap-1.5 text-[10px] font-bold text-muted uppercase opacity-70">
                                                        ...
                                                        <span className="tabular-nums">{formatDate(student.birth_date as string)}</span>
                                                        {calculateAge(student.birth_date as string) !== null && (
                                                            <span className="text-[10px]">
                                                                ({calculateAge(student.birth_date as string)} {t.years})
                                                            </span>
                                                        )}
                                                    </div>
                                                )}
                                        ...
                                        {!!student.dance_style && (
                                            <div className="flex items-center gap-1.5 text-[11px] text-[#6d28d9] font-bold">
                                                ...
                                                <span>{student.dance_style as string}</span>
                                            </div>
                                        )}
                                    </div>
                                </div>
            ...
            {hasNextPage && (
                <div className="flex justify-center pb-4">
                    <button onClick={() => fetchNextPage()} disabled={isFetchingNextPage}
                        className="px-6 py-3 rounded-2xl bg-surface border border-border-subtle text-xs font-black uppercase tracking-widest text-muted hover:text-primary hover:border-[#6d28d9]/30 transition-all disabled:opacity-50 flex items-center gap-2">
                        {isFetchingNextPage && <Loader2 className="w-4 h-4 animate-spin" />}
                        {`მეტის ჩვენება (${rows.length} / ${total})`}
                    </button>
                </div>
            )}
