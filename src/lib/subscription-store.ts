...
// Distinct from getEffectiveStatus() above: that one collapses "paused" and
// "overdue" into a single 'suspended' bucket (with a reason/days note) for
// the subscription list UI; this one keeps all 4 buckets separate as plain
// strings, for the dashboard's status-breakdown donut chart.
export function getSubscriptionStatusBucket(sub: SubscriptionInfo): SubscriptionEffectiveStatus {
