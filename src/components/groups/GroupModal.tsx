...
import { X, UserRound, Trash2, AlertTriangle, Check, Palette, Clock, Calendar, Save, ArrowRight, User, BookOpen, GraduationCap, Plus, DoorOpen } from 'lucide-react';
            ...
            <div className={cn(
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-muted mt-0.5 font-medium opacity-70 tracking-tight">
                                {isEdit ? group!.name : t.addGroupDescription}
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="space-y-4">
                                ...
                                {[0, 1, 2, 3, 4, 5, 6].map(d => {
                                    ...
                                    return (
                                        <button key={d} onClick={() => toggleDaySlot(d)}
                                            ...
                                            style={isActive ? { backgroundColor: form.color, borderColor: form.color } : {}}>
                                            {dayLabels[d]}
                                        </button>
                                    );
                                })}
                        ...
                        {slots.length > 0 && (
                            <div className="flex items-center gap-2 px-3 py-2 rounded-xl"
                                 style={{ backgroundColor: `${form.color}08`, border: `1px solid ${form.color}20` }}>
                                <Calendar className="w-3.5 h-3.5 flex-shrink-0" style={{ color: form.color }} />
                                <p className="text-[10px] font-bold opacity-80" style={{ color: form.color }}>
                ...
                <div className="px-5 py-4 border-t border-border-subtle bg-white/90 backdrop-blur-md sticky bottom-0 z-10 pb-10 sm:pb-8 flex-shrink-0">
                    {isEdit && !showDelete && !isTeacher && (
                        <button onClick={() => setShowDelete(true)} className="w-full mb-4 py-2 text-red-500/60 hover:text-red-500 text-[10px] sm:text-xs font-bold border border-red-500/10 hover:border-red-500/30 rounded-xl transition-all flex items-center justify-center gap-2 text-center px-2">
                    ...
                    {showDelete && (
                        <div className="mb-4 p-4 bg-red-500/10 border border-red-500/20 rounded-2xl flex items-center gap-4 animate-in slide-in-from-top-2 duration-300">
                            ...
                            <button onClick={() => { if (group?.id) onDelete?.(group.id); onClose(); }} className="px-3 py-1 sm:px-4 sm:py-1.5 bg-red-500 text-white text-[10px] sm:text-[11px] font-bold rounded-lg hover:bg-red-600 active:scale-95 transition-all">{t.delete}</button>
                    ...
                    <div className="flex gap-3">
                        ...
                        <button onClick={save} disabled={!form.name || saving} 
                                className="flex-1 py-3 disabled:opacity-40 text-white rounded-xl font-black text-[11px] sm:text-xs active:scale-95 transition-all flex items-center justify-center gap-2 uppercase tracking-widest"
                                style={{ 
                                    backgroundColor: form.color,
                                    boxShadow: `0 8px 20px -4px ${form.color}40`
                                }}>
