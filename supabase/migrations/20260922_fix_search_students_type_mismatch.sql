-- Second layer of the same /students outage, found only after
-- 20260922_fix_search_students_ambiguous_id.sql removed the first bug (that
-- one raised its error before RETURN QUERY ever executed, so this one was
-- never reached/observed before now — meaning /students has likely never
-- actually returned data in production).
--
-- "structure of query does not match function result type": the function
-- declares `RETURNS TABLE (..., sub_expires_at date, ...)`, but
-- subscriptions.expires_at is actually `timestamp with time zone`
-- (confirmed via information_schema.columns), not `date`. Every other
-- column matches (students.id/full_name/first_name/last_name/phone/email
-- are all `text`, students.data is `jsonb`, subscriptions.sessions_total/
-- sessions_used are `integer`, subscriptions.status is `text`).
--
-- Fix: cast expires_at to date right where it's selected in the LATERAL
-- subquery, so every downstream reference (the status/remaining-sessions
-- comparisons, and the final RETURN QUERY column) consistently carries
-- `date`, matching the declared return type.
...
BEGIN
    ...
    WITH matched AS (
        ...
        LEFT JOIN LATERAL (
            SELECT status, sessions_total, sessions_used, expires_at::date AS expires_at
