    ...
    return (
        <>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                <div className="w-full max-w-sm bg-[#13131a] border border-white/[0.10] rounded-2xl shadow-2xl">
                    {/* Header */}
                    <div className="flex items-center justify-between px-5 py-4 border-b border-white/[0.08]">
                        <div className="flex items-center gap-3">
                            ...
                            <h2 className="text-sm font-bold text-white">{isEdit ? 'დარბაზის ჩასწ.' : 'ახალი დარბაზი'}</h2>
                        </div>
                        <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-xl hover:bg-white/[0.07] text-white/40"><X className="w-4 h-4" /></button>
                    </div>

                    {/* Body */}
                    <div className="px-5 py-5 space-y-4">
                        <div>
                            <label className="text-xs text-white/40 mb-1.5 block">სახელი *</label>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <div>
                                <label className="text-xs text-white/40 mb-1.5 flex items-center gap-1.5 block"><Users className="w-3 h-3" /> ტევადობა</label>
                            ...
                            <div>
                                <label className="text-xs text-white/40 mb-1.5 block">სტატუსი</label>
                                <button onClick={() => setF('is_active', !form.is_active)}
                                    className={cn('w-full py-2.5 px-3 rounded-xl text-xs font-semibold border transition-all',
                                        form.is_active ? 'bg-emerald-500/10 border-emerald-500/25 text-emerald-300' : 'bg-white/[0.04] border-white/[0.10] text-white/40')}>
                        ...
                        <div>
                            <label className="text-xs text-white/40 mb-2 flex items-center gap-1.5 block"><Palette className="w-3 h-3" /> ფერი</label>
                                ...
                                {COLORS.map(c => (
                                    <button key={c.hex} onClick={() => setF('color', c.hex)} title={c.label}
                                        ...
                                        style={{
                                            ...
                                            borderColor: form.color === c.hex ? '#ffffff' : 'transparent',
                        ...
                        <div>
                            <label className="text-xs text-white/40 mb-1.5 block">აღწერა</label>
                    ...
                    <div className="px-5 pb-5 space-y-2">
                        {isEdit && !showDelete && (
                            <button onClick={() => setShowDelete(true)}
                                className="w-full py-2 text-red-400/60 hover:text-red-400 text-xs font-medium border border-red-500/10 hover:border-red-500/30 rounded-xl transition-all flex items-center justify-center gap-2">
                        ...
                        {showDelete && (
                            <div className="p-3 bg-red-500/10 border border-red-500/20 rounded-xl flex items-center gap-3">
                                ...
                                <button onClick={() => { onDelete?.(hall!.id); onClose(); }} className="text-xs font-bold text-red-400">წაშლა</button>
                                <button onClick={() => setShowDelete(false)} className="text-xs text-white/40">გაუქ.</button>
                            </div>
                        )}
                        <div className="flex gap-2">
                            <button onClick={onClose}
                                className="flex-1 py-2.5 border border-white/[0.10] text-white/60 text-sm font-medium rounded-xl hover:border-white/[0.2] transition-colors">
                                გაუქმება
                            </button>
                            <button onClick={save} disabled={!form.name?.trim()}
                                className="flex-1 py-2.5 bg-violet-500 hover:bg-violet-600 disabled:opacity-40 text-white text-sm font-semibold rounded-xl transition-colors flex items-center justify-center gap-2">
